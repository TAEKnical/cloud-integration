...
![HTTP%201e46b7039eaf4530a8f8b9fd4ff354d0/Untitled.png](HTTP%201e46b7039eaf4530a8f8b9fd4ff354d0/Untitled.png)

하이퍼텍스트 컨셉연결점을 ‘링크’
우리가 익숙한건 하이퍼링크기능. 마우스 오른쪽.
익숙해서 의식하지 못한 것 뿐
...
HTTP는 TCP 기반의 프로토콜
따라서 HTTP 요청/응답이 오가기 전, 후로 TCP connection 과 termination 절차가 필요.
즉, TCP termination이 발생하므로 기존의 연결이 유지되지 않음. 연결이 유지될수록 서버에는 관리 대상이 누적되기 때문.

[2] Stateless (상태 비 저장)

인증수단이 필요한 이유

바로 직전에 처리된 통신에 대해서 알 수 없다는 성질

하이퍼링크를 누를 때 마다, 바로 직전의 상태를 알 수 없으므로 로그인을 했더라도 했는지 안했는지 다음 페이지에서는 알 수가 없다.
그렇다고 매번 HTTP 패킷에 로그인 정보를 담아 주고받을 수는 없다.
로그인만이 문제가 아니라 stateless로 인해 발생하는 모든 상황을 전부 이런 방식으로 다루었다면 HTTP는 지금까지 쓰이지 않았을 것.

문제만 있느냐. 그것도 아님.
각 통신을 독립적으로 다룰 수 있다는 장점.
